...
test_set = set()
for item in con_d:
    perms = itertools.combinations(con_d[item], 3)
    for perm in perms:
        p1, p2, p3 = sorted(perm)
        test_set.add((p1, p2, p3))

print(len(test_set))

'''    for p1, p2, p3 in perms:
        if p1 in con_d and p2 in con_d and p3 in con_d:  # Check that all p's are left in con_d.
            if p2 in con_d[p1] and p3 in con_d[p1]:
                if p1 in con_d[p2] and p3 in con_d[p2]:
                    if p1 in con_d[p3] and p2 in con_d[p3]:
                            print('Winner', item + p1 + p2 + p3, item, p1, p2, p3)
                            break
'''
